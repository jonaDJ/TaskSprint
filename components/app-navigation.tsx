  ...
  return (
    <aside className={`sidebar page-${active} ${open ? "" : "collapsed"}`}>
      ...
      <nav className="primary-nav" aria-label="Main navigation">
        ...
        <Link className={active === "reports" ? "active" : ""} href="/reports" aria-label="Reports" title="Reports" aria-current={active === "reports" ? "page" : undefined}><ChartIcon /><span>Reports</span></Link>
      </nav>
      {children}
      <CloudAccount />
    </aside>
...
export function MobileNavigation({ active }: { active: Page }) {
  return (
    <nav className="mobile-nav" aria-label="Main navigation">
      ...
      <Link className={active === "reports" ? "active" : ""} href="/reports" aria-current={active === "reports" ? "page" : undefined}><ChartIcon /><span>Reports</span></Link>
      <CloudAccount mobile />
    </nav>
  );
}
